...
class BranchManager {
    ...
    async refreshEditorContentAfterEvaluation(promptId) {
        try {
            ...
            if (isExpanded && this.editorManager) {
                ...
                // Recreate the editor with the latest prompt data (to decide diff vs editor)
                const promptData = (this.dataService && typeof this.dataService.getCachedPrompts === 'function')
                    ? (this.dataService.getCachedPrompts() || []).find(p => p && p.id === promptId)
                    : null;
                await this.editorManager.ensureEditorForBranch(promptId, promptData);
    ...
    async initializeBranchComponents(id) {
        try {
            // Lazy init editor after expand, pass prompt data for editor decision (diff vs editor)
            const promptData = (this.dataService && typeof this.dataService.getCachedPrompts === 'function')
                ? (this.dataService.getCachedPrompts() || []).find(p => p && p.id === id)
                : null;
            await this.editorManager.ensureEditorForBranch(id, promptData);
        } catch (err) {
