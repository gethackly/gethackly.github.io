...
class PanelManager {
    ...
    expandPanel(target) {
        ...
        // Notify listeners about panel expansion for optimization controls
        try {
            const name = target.getAttribute('data-panel');
            window.dispatchEvent(new CustomEvent('panel-expanded', { detail: { name } }));
        } catch (_) {}
    }
