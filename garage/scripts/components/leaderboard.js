...
class LeaderboardManager {
    ...
    getDisplayName(it) {
        // Prioritize githubUsername, then username, then display_name
        const uname = it.githubUsername || it.username || it.display_name || '';
        if (uname && uname !== it.email) return uname;
        // Never fallback to email - use generic name instead
        return '—';
    }
    ...
    getUsernameLink(it) {
        ...
        // Use githubUsername if available, otherwise fallback to username
        const githubUsername = it.githubUsername || it.username || it.display_name || '';
        if (githubUsername && githubUsername !== it.email) {
    ...
    render(items) {
        ...
        for (const it of items) {
            ...
            const usernameLink = this.getUsernameLink(it);
            ...
            row.innerHTML = `
                ...
                <div class="leaderboard-col leaderboard-col-user">
                    ...
                    <span class="lb-name">${usernameLink}</span>
                </div>
