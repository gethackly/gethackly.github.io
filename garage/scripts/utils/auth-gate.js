...
class AuthGate {
    ...
    showAuthPrompt(actionName, customMessage = null) {
        ...
        promptHeader.className = 'auth-modal-header';
        Object.assign(promptHeader.style, {
            position: 'relative',
            width: '100%',
            height: '40px',
        ...
        promptHeader.innerHTML = '<button class="auth-close-btn" onclick="window.authGate.closePrompt()">×</button>';

        // Create content section
        const promptBody = document.createElement('div');
        promptBody.className = 'auth-modal-body';
        ...
        promptBody.innerHTML = `
            <h2 style="margin: 0 0 16px 0; font-size: 20px;">Sign In Required</h2>
            <p style="color: var(--color-text-secondary); margin: 0 0 20px 0; font-size: 14px;">${message}</p>
