...
class UsernameValidator {
    ...
    validate(username) {
        ...
        // Check if reserved
        const reservedResult = this.validateReserved(username);
        if (!reservedResult.valid) {
            return reservedResult;
        }

        // Check for profanity
        const profanityResult = this.validateProfanity(username);
    ...
    // Get username suggestions based on input
    getSuggestions(input) {
        ...
        const suggestions = [];
        const base = input.toLowerCase().replace(/[^a-zA-Z0-9]/g, '');
        ...
        // Add common suffixes
        const suffixes = ['dev', 'user', 'pro', 'master', 'ninja', 'guru', 'ace', 'star'];
        ...
        return suggestions.filter(suggestion => {
            const result = this.validate(suggestion);
            return result.valid;
